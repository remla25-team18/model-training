...
def test_reproducibility(paths_setup):
    ...
    assert all(pred in set(y_test) for pred in y_pred_1), "Some predictions are not in the set of unique labels"
    ...
    assert all(pred in set(y_test) for pred in y_pred_2), "Some predictions are not in the set of unique labels"
